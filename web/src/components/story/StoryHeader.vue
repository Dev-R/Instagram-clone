    ...
    setup(prop, context) {
         ...
         */
         const playStoryVideo = () => {
            if (prop.activeStoryType === 'Video') {
                const video = prop.activeStory as HTMLVideoElement
                video?.play()
        ...
        */
         ...
         */
         watchEffect(() => {
            isStoryPlaying.value ? context.emit("onResumeStory") : context.emit("onPauseStory")
        })

        
        watchEffect(() => {
            prop.activeStoryType === 'Video' ? playStoryVideo() : muteStory()
        })
